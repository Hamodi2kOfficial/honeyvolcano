    ...
    >
      ...
      >
        ...
        <div aria-hidden className="absolute inset-0 z-0" style={{
          background: "radial-gradient(ellipse at 50% 35%, rgba(255,255,255,0.05) 0%, rgba(21,21,21,1) 75%)",
        }} />

        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-3 md:gap-4 font-display">
          <h2 ref={titleTopRef} aria-hidden className="font-normal uppercase text-[#D4AF37]" style={{
            fontSize: "clamp(3.75rem, 12vw, 11rem)", lineHeight: 0.85, letterSpacing: "0.02em",
          }}>{titleTop}</h2>

          <div ref={cardRef} className="relative overflow-hidden rounded-[12px] shadow-[0_20px_80px_rgba(0,0,0,0.85)] ring-1 ring-[#D4AF37]/30 will-change-transform md:rounded-[16px]" style={{
            width: `min(96vw, calc(72svh * ${aspect}))`,
            height: `min(72svh, 96vw / ${aspect})`,
            aspectRatio: aspect,
          }}>
            <div aria-hidden className="pointer-events-none absolute inset-0 z-20 shadow-[inset_0_0_120px_rgba(0,0,0,0.65)]" />
            {framesOk && (
              <canvas ref={canvasRef} aria-hidden className="absolute inset-0 h-full w-full object-cover" />
            )}
          </div>

          <h2 ref={titleBottomRef} aria-hidden className="font-normal uppercase text-[#D4AF37]" style={{
            fontSize: "clamp(3.75rem, 12vw, 11rem)", lineHeight: 0.85, letterSpacing: "0.02em",
          }}>{titleBottom}</h2>
        </div>
      </div>
